...
def vdi(vdi_info):
    # return ['Success', ['Vdi', {'vdi': location, 'virtual_size':
    # str(virtual_size) }]]
...
class Marshall:
    ...
    def sr_attach(self, args):
        result = self.x.sr_attach(
            args["task"], args["sr"], args["device_config"])
        expect_none(result)
        return value(unit)
    ...
    def vdi_create(self, args):
        vi = self.x.vdi_create(args["task"], args["sr"], vdi_info(
            args["vdi_info"]), args["params"])
    ...
    def vdi_attach(self, args):
        result = self.x.vdi_attach(args["task"], args["dp"], args[
                                   "sr"], args["vdi"], args["read_write"])
        expect_string(result)
        return value(params(result))

    def vdi_activate(self, args):
        result = self.x.vdi_activate(
            args["task"], args["dp"], args["sr"], args["vdi"])
        expect_none(result)
        return value(unit)

    def vdi_deactivate(self, args):
        result = self.x.vdi_deactivate(
            args["task"], args["dp"], args["sr"], args["vdi"])
        expect_none(result)
        return value(unit)

    def vdi_detach(self, args):
        result = self.x.vdi_detach(
            args["task"], args["dp"], args["sr"], args["vdi"])
        expect_none(result)
        return value(unit)
...
class Server(SimpleXMLRPCServer):
    ...
    def __init__(self, ip, port):
        SimpleXMLRPCServer.__init__(
            self, (ip, port), requestHandler=RequestHandler)
