...
def open_pif_ifcfg(pif):
    ...
    f = ConfigurationFile(
        "%s/etc/sysconfig/network-scripts/ifcfg-%s" % (root_prefix(), interface))
...
def bring_down_interface(pif, destroy=False):
    ...
    if pif_is_vlan(pif):
        ...
        masters = [m for m in masters if m != pif and db().get_pif_record(m)[
            'currently_attached']]
        if len(masters) > 0:
        ...
        log("vlan masters of %s - %s" % (db().get_pif_record(pif)
                                         ['device'], [pif_netdev_name(m) for m in vlan_masters]))
    ...
    log("bond slaves of %s - %s" % (db().get_pif_record(pif)
                                    ['device'], [pif_netdev_name(s) for s in bond_slaves]))
...
def interface_is_up(pif):
    try:
        interface = pif_netdev_name(pif)
        state = open("%s/sys/class/net/%s/operstate" %
                     (root_prefix(), interface)).read().strip()
        return state == "up"
    except:
...
def pif_get_bond_slaves_sorted(pif):
    ...
    try:
        attached_slaves = open("%s/sys/class/net/%s/bonding/slaves" %
                               (root_prefix(), pifrec['device'])).readline().split()
        for slave in attached_slaves:
            pifs = [p for p in db().get_pifs_by_device(slave)
