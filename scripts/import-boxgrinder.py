...
def import_vdi(url, session, vdi, filename):
    cmd = "%s -T%s %s/import_raw_vdi?session_id=%s\&vdi=%s" % (
...
if __name__ == "__main__":
    ...
    parser.add_option("-u", "--username", dest="username",
                      help="login as USERNAME", metavar="USERNAME")
    ...
    session = value(server.session.login_with_password(settings["username"], settings[
                    "password"], "1.0", "xen-api-scripts-import-boxgrinder"))
    try:
