...
void check_address(char* buffer, char* reply) {
    if (buffer != reply) {
        ...
        syslog(LOG_MAKEPRI(LOG_USER,LOG_ERR),
                "pciutils: nothing to complain about. Address is %p.\n", buffer);
        fprintf(stderr, "[pciutils] nothing to complain about. Address is %p.\n", buffer);
        fflush(stderr);
    }
}
