...
let finalise (carray : char Ctypes.CArray.t) =
  Printf.printf "Deallocating 0x%nx\n" Ctypes.(raw_address_of_ptr @@ to_voidp @@ CArray.start carray)
...
let lspci_nnnDv pci_access =
  ...
  List.iter (fun d ->
    ...
    Printf.printf "Class:  %s [%04x]\n"
      (lookup_class_name ~finalise pci_access d.device_class |> default) d.device_class;
    Printf.printf "Vendor: %s [%04x]\n"
      (lookup_vendor_name ~finalise pci_access d.vendor_id |> default) d.vendor_id;
    Printf.printf "Device: %s [%04x]\n"
      (lookup_device_name ~finalise pci_access d.vendor_id d.device_id |> default) d.device_id;
    begin match d.subsystem_id with
    | Some (sv_id, sd_id) ->
      Printf.printf "SVendor:\t%s [%04x]\n"
        (lookup_subsystem_vendor_name ~finalise pci_access sv_id |> default) sv_id;
      Printf.printf "SDevice:\t%s [%04x]\n"
        (lookup_subsystem_device_name ~finalise pci_access d.vendor_id d.device_id sv_id sd_id |> default) sd_id
    | None -> ()
    end;
  ...
  let n = lookup_subsystem_device_name ~finalise pci_access nv_vid k1_did nv_vid id_160
          |> default in
  Printf.printf "\"%s\"\n" n
